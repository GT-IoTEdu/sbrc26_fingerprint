...
- **`policy`**: `--canon_policy` do `iot_id_fingerprint.py` ou `--policy` no `fingerprint_hash.py`: **`stable`** (padrão) ou **`rich`**.

Implementação: `canonicalize_features.build_canon` + `dumps_canon`; hash em `iot_id_fingerprint.py` / `fingerprint_hash.py`.
...
| Hash | `hashlib.sha256(canon_str.encode("utf-8")).hexdigest()` em `iot_id_fingerprint.py` ou `fingerprint_hash.compute_hash_from_canon_string` |
